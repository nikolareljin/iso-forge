...
grep -Fq 'ventoy_system_cache_dir()' "$ROOT_DIR/inc/isoforge.sh"
grep -Fq '"${elevate[@]}" tar --no-same-owner --no-same-permissions -xzf "$tmpdir/ventoy.tgz" -C "$root_cache"' "$ROOT_DIR/inc/isoforge.sh"
! grep -Fq '"$cache_dir"/ventoy-*/Ventoy2Disk.sh' "$ROOT_DIR/inc/isoforge.sh"
grep -Fq 'ventoy_download_file()' "$ROOT_DIR/inc/isoforge.sh"
grep -Fq 'cleanup_owned_ventoy_mount' "$ROOT_DIR/inc/isoforge.sh"
grep -Fq 'validate_selected_images || return 1' "$ROOT_DIR/inc/isoforge.sh"
grep -Fq 'local -a elevate=()' "$ROOT_DIR/inc/isoforge.sh"
